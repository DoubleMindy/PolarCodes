...
ScListFlipStatDecoder::ScListFlipStatDecoder(PolarCode * codePtr, int L, double omega) : ScListDecoder(codePtr, L) {
	...
	if (isUsedStat) {
		...
		if (isDoubleFromSingles) {
			...
			for (std::size_t i = 0; i < _singleFlips.size(); i++)
			{
				for (std::size_t j = i + 1; j < _singleFlips.size(); j++)
				{
		...
		_doubleFlips = std::vector<std::tuple<int, int>>();
		for (std::size_t i = 0; i < unfrozenBitsWithCrc.size(); i++)
		{
			for (std::size_t j = i + 1; j < unfrozenBitsWithCrc.size(); j++)
			{
